...
function validateEmailInput(input) {
  const { nome, email, zap, destination } = input;
  ...
  if (
    ...
    return "Destino desejado é obrigatória e deve ser um texto válido.";
  }
...
app.post("/send-email", async (req, res) => {
  ...
  const { nome, email, zap, destination } = req.body;
  console.log(req.body);

  try {
    const mailOptions = {
      from: `"${nome}" <${email}>`,
      to: "newsletter_tbv@gmail.com",
      subject: `${nome} se increveu para receber descontos!`,
      html: `
    <html>
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 8px; background-color: #f9f9f9;">
        ...
        <div style="text-align: center; margin-bottom: 20px;">
          <h1 style="color: blue;><span style="color: #999;>TravelBuena</span>Vista</h1>
        </div>
        ...
        <p><strong>Por onde nos encontrou:</strong> ${destination}</p>
